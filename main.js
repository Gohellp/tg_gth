...
const megaRegex = new RegExp(
	bot.platforms.map((p) => p.fullRegex.source).join("|"),
	"gmi",
);

bot.start((ctx) => {
	ctx.reply(
		"Send me url on song on YouTube or SoundCloud and i send you audio file!\nSend me audio file and i try to find lyrics!",
	);
});
bot.url(megaRegex, async (ctx) => {
	...
	if (links.length === 0) {
		return ctx.reply(
			"I think this isn't souncloud or youtube url. Try again ;)",
		);
	}
...
bot.on(message("audio"), async (ctx) => {
	...
	let lyrics = await getLyrics({
		apiKey: genius_tk,
		artist: ctx.message.audio.performer.replace(
			/ ?\((\w)*.? ? (\w?[А-Яа-я]?)*\)/iu,
			"",
		),
		title: ctx.message.audio.title.replace(
			/ ?\((\w)*.? ? (\w?[А-Яа-я]?)*\)/iu,
			"",
		),
		optimizeQuery: true,
	...
	await ctx.reply(
		lyrics
			.replace(
				/\[(«?[\p{Alpha}\p{M}\p{Nd}\p{Pc}\p{Join_C}]*-*?\s*?\d*?»?)*?\]/gu,
				"",
			)
			.slice(0, 4093),
	);
